...
- **内置指令用户白名单 `builtin_command_users`**
  - 新增配置项 `builtin_command_users`（在「消息过滤」分组下）
  - 类型：用户ID列表，默认为空
  - 留空表示所有用户均可使用 Hermes 内置指令（`/new`、`/help`、`/status` 等）
  - 填写用户ID后，仅列表中的用户可以转发内置指令，其他人发送的内置指令会被静默拦截（调用 `event.stop_event()` 阻止事件传播）
  - 与 `forward_builtin_commands` 开关配合使用：开关关闭时此配置不生效，开关开启时按此列表过滤用户
